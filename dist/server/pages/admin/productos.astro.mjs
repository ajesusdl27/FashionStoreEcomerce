import { f as createComponent, k as renderComponent, r as renderTemplate, m as maybeRenderHead, l as renderScript, h as addAttribute } from '../../chunks/astro/server_IieVUzOo.mjs';
import 'piccolore';
...
const $$Index = createComponent(async ($$result, $$props, $$slots) => {
  ...
  return renderTemplate`${renderComponent($$result, "AdminLayout", $$AdminLayout, { "title": "Productos" }, { "default": async ($$result2) => renderTemplate` ${maybeRenderHead()}<div class="space-y-6"> <!-- Header --> <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4"> <p class="text-muted-foreground" id="products-count"> ${products?.length || 0} productos en total
...
Error cargando productos: ${error.message} </div>`} <!-- Products List as Cards (Better for this design) --> <div class="space-y-2" id="products-list"> ${products && products.length > 0 ? products.map((product) => {
    ...
    return renderTemplate`<div class="product-row bg-card hover:bg-muted/50 border border-border rounded-lg p-4 transition-all"${addAttribute(product.name.toLowerCase(), "data-name")}${addAttribute(product.slug.toLowerCase(), "data-slug")}${addAttribute(product.category?.id || "", "data-category-id")}${addAttribute(product.active ? "true" : "false", "data-active")}${addAttribute(product.is_offer ? "true" : "false", "data-offer")}${addAttribute(totalStock, "data-stock")}> <div class="flex items-center gap-4">  <div class="flex-shrink-0"> <img${addAttribute(getFirstImage(product.images), "src")}${addAttribute(product.name, "alt")} class="w-16 h-16 object-cover rounded-lg bg-muted ring-1 ring-border"> </div>  <div class="flex-1 min-w-0 grid grid-cols-1 md:grid-cols-5 gap-4 items-center">  <div class="md:col-span-2"> <p class="font-semibold text-foreground truncate"> ${product.name} </p> <p class="text-xs text-muted-foreground truncate"> ${product.slug} </p> </div>  <div> <span class="inline-flex items-center px-2.5 py-1 rounded-md text-xs font-medium bg-muted text-muted-foreground"> ${product.category?.name || "Sin categor\xEDa"} </span> </div>  <div class="text-right"> ${product.is_offer && product.offer_price ? renderTemplate`<div> <p class="text-xs text-muted-foreground line-through"> ${formatPrice(product.price)} </p> <p class="font-bold text-red-600 dark:text-red-400"> ${formatPrice(product.offer_price)} </p> </div>` : renderTemplate`<p class="font-bold tabular-nums text-foreground"> ${formatPrice(product.price)} </p>`} </div>  <div class="flex items-center justify-end gap-2 flex-wrap"> <span${addAttribute(`inline-flex items-center px-2.5 py-1 rounded-md text-xs font-semibold ${totalStock === 0 ? "bg-red-500/20 text-red-600 dark:text-red-400" : totalStock < 10 ? "bg-yellow-500/20 text-yellow-600 dark:text-yellow-400" : "bg-green-500/20 text-green-600 dark:text-green-400"}`, "class")}> ${totalStock} uds
</span> ${product.active ? renderTemplate`<span class="inline-flex items-center px-2.5 py-1 rounded-md text-xs font-semibold bg-green-500/20 text-green-600 dark:text-green-400">
Activo
</span>` : renderTemplate`<span class="inline-flex items-center px-2.5 py-1 rounded-md text-xs font-semibold bg-muted text-muted-foreground">
Inactivo
</span>`} ${product.is_offer && renderTemplate`<span class="inline-flex items-center px-2.5 py-1 rounded-md text-xs font-semibold bg-red-500/20 text-red-600 dark:text-red-400">
Oferta
</span>`} </div> </div>  <div class="flex items-center gap-1 flex-shrink-0"> <a${addAttribute(`/admin/productos/${product.id}`, "href")} class="p-2 rounded-lg hover:bg-muted transition-colors" title="Editar"> <svg class="w-5 h-5 text-muted-foreground hover:text-foreground" fill="none" stroke="currentColor" viewBox="0 0 24 24"> <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path> </svg> </a> <a${addAttribute(`/productos/${product.slug}`, "href")} target="_blank" class="p-2 rounded-lg hover:bg-muted transition-colors" title="Ver en tienda"> <svg class="w-5 h-5 text-muted-foreground hover:text-foreground" fill="none" stroke="currentColor" viewBox="0 0 24 24"> <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14"></path> </svg> </a> </div> </div> </div>`;
  }) : renderTemplate`<div class="text-center py-12 text-muted-foreground"> <svg class="w-16 h-16 mx-auto mb-4 opacity-30" fill="none" stroke="currentColor" viewBox="0 0 24 24"> <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4"></path> </svg> <p>No hay productos todavía</p> <a href="/admin/productos/nuevo" class="inline-block mt-4 text-primary hover:underline">
Crear primer producto →
</a> </div>`} </div> <!-- No results message (hidden by default) --> <div id="no-results" class="hidden p-12 text-center text-muted-foreground bg-muted/30 rounded-lg border border-border"> <svg class="w-16 h-16 mx-auto mb-4 opacity-30" fill="none" stroke="currentColor" viewBox="0 0 24 24"> <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"></path> </svg> <p>No se encontraron productos</p> <button id="clear-filters" class="mt-4 text-primary hover:underline">
Limpiar filtros
