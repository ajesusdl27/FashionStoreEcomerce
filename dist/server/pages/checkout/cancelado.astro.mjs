...
const $$Cancelado = createComponent(async ($$result, $$props, $$slots) => {
  const Astro2 = $$result.createAstro($$Astro, $$props, $$slots);
  Astro2.self = $$Cancelado;
  const orderId = Astro2.url.searchParams.get("order_id");
  let isProcessed = false;
  if (orderId) {
    try {
      const { data: order } = await supabase.from("orders").select("stripe_session_id, status").eq("id", orderId).single();
  ...
  return renderTemplate`${renderComponent($$result, "PublicLayout", $$PublicLayout, { "title": "Pago Cancelado - FashionStore" }, { "default": async ($$result2) => renderTemplate` ${maybeRenderHead()}<div class="container mx-auto px-4 py-12"> <div class="max-w-xl mx-auto text-center"> <!-- Cancel Icon --> <div class="w-24 h-24 mx-auto mb-8 bg-accent/20 rounded-full flex items-center justify-center"> <svg class="w-12 h-12 text-accent" fill="none" stroke="currentColor" viewBox="0 0 24 24"> <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path> </svg> </div> <h1 class="font-display text-4xl md:text-5xl mb-4">Pago cancelado</h1> <p class="text-xl text-muted-foreground mb-8"> ${isProcessed ? "Hemos cancelado tu pedido y restaurado el stock. Puedes volver a intentarlo cuando quieras." : "No te preocupes, tu carrito sigue intacto. Puedes volver a intentarlo cuando quieras."} </p> <div class="bg-card border border-border rounded-xl p-6 mb-8"> <h2 class="font-heading font-semibold mb-4">¿Necesitas ayuda?</h2> <p class="text-muted-foreground text-sm mb-4">
...
(+34) 912 345 678
</li> </ul> </div> <div class="flex flex-col sm:flex-row gap-4 justify-center"> ${renderComponent($$result2, "Button", $$Button, { "href": "/carrito", "variant": "primary", "size": "lg" }, { "default": async ($$result3) => renderTemplate`
Volver al carrito
` })} ${renderComponent($$result2, "Button", $$Button, { "href": "/productos", "variant": "secondary", "size": "lg" }, { "default": async ($$result3) => renderTemplate`
Seguir comprando
